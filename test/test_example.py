...
class TestExample(DbTest):
    ...
    def test_count_the_number_of_subordinates(self, conn):
        ...
        FROM
          ...
          LEFT JOIN
            (
              SELECT
                COUNT(sales_organization_id) AS so_num,
                sales_organization_id AS so_id 
              FROM
    ...
    def test_segments_using_geojson_boundary(self, conn):
        ...
        FROM
           japan_segments 
        WHERE
            ST_Within( japan_segments.bounds, ST_SetSRID(ST_GeomFromGeoJSON('{"type": "Polygon", 
            "coordinates":[[[130.27313232421875, 30.519681272749402], [131.02020263671875, 30.519681272749402], [131.02020263671875, 30.80909017893796], [130.27313232421875, 30.80909017893796], [130.27313232421875, 30.519681272749402]]]}'), 4326)
        );
        """
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(sql)
